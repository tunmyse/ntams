...
class College_model extends CI_Model {
    ...
    public function get_college($id = NULL) {
        ...
        if($id == NULL) {
            return $this->util_model->get_data($table_name, array('*'), array(), $order);
        } else {
            ...
            if(is_int($id) && $id > 0) {
                ...
                $where = array(
                            array('field' => 'colid', 'value' => $id)
                        );
                $ret = $this->util_model->get_data($table_name, array('*'), $where, $order);
                
                if($ret === DEFAULT_EMPTY)
